TODO
  - [ ] Environment Variable setup bash file
	  - [ ]  need to organize this file for ease of use
	  - [ ] `cyclonedds.xml` file 
	  - [ ] Guide for zero copy feature
# How to install CycloneDDS and turn on shared memory feature
> [!NOTE] Use official install and then just have a separate configuration
>   `sudo apt install ros-humble-rmw-cyclonedds-cpp`
>  Then, you need:
>  cyclonedds.xml to have the following:
>  
```xml
...
# Introspection tool installaion

> [!Note] You need to first finish rmw_cyclonedds setup from above
> 

> [!WARNING] You might have this error
>> 2026-01-14 19:08:07.094 [ Error ]: Version mismatch from 'introspection'! Please build your app and RouDi against the same iceoryx version (version & commitID). RouDi: 1:21:01:51:020:2025-07-01T01:17:51Z0: App: 1:21:01:61:020:2026-01-14T10:06:46Z12:41b69258cf48

> [!NOTE] Reason
>  the classic Iceoryx **Version & CommitID mismatch**.
>  Iceoryx is extremely strict. It doesn't just check the version number (e.g., `2.0`); it hashes the **Commit ID** and **Build Timestamp** into the connection handshake. Because you just built the introspection tool today (2026-01-14), its "ID" doesn't match the RouDi daemon that came with your `apt` install (built 2025-07-01).


So, you need to check the exact version downloaded by apt:
```bash
apt-cache policy ros-humble-iceoryx-posh
ros-humble-iceoryx-posh:
  Installed: 2.0.5-1jammy.20250701.011751
  Candidate: 2.0.5-1jammy.20250701.011751
  Version table:
 *** 2.0.5-1jammy.20250701.011751 500
...
Target tag `v2.0.5` :
```git
git clone -b v2.0.5 https://github.com/eclipse-iceoryx/iceoryx.git
cd iceoryx
```


> [!NOTE] Attention
> Iceoryx uses a "meta-project" structure. The main logic and the introspection tool are actually located inside the `iceoryx_posh` subdirectory.
> To build the tool specifically, you need to point CMake to `iceoryx_meta` or `iceoryx_posh`
...
```bash
cmake ../iceoryx_meta -DINTROSPECTION=ON
```

```bash
cmake --build . --target iox-introspection-client
```
...
2026-01-14 20:30:25.516 [ Info  ]: No config file provided. Using '/etc/iceoryx/roudi_config.toml'
Log level set to: [Verbose]
2026-01-14 20:30:25.516 [Verbose]: Command line parameters are:
Log level: Verbose
...
2026-01-14 20:30:25.550 [ Debug ]: Registered memory segment 0x747026a54000 with size 66761736 to id 1
Reserving 149264720 bytes in the shared memory [ted]
[ Reserving shared memory successful ] 
2026-01-14 20:30:25.621 [ Debug ]: Roudi registered payload data segment 0x74701dbfa000 with size 149264720 to id 2
RouDi is ready for clients
2026-01-14 20:30:25.645 [ Debug ]: Registered new application introspection
2026-01-14 20:30:25.658 [ Debug ]: Created new SubscriberPort for application 'introspection' with service description 'Service: Introspection, Instance: RouDi_ID, Event: MemPool'
2026-01-14 20:30:25.659 [ Debug ]: Created new SubscriberPort for application 'introspection' with service description 'Service: Introspection, Instance: RouDi_ID, Event: Process'
2026-01-14 20:30:25.659 [ Debug ]: Created new SubscriberPort for application 'introspection' with service description 'Service: Introspection, Instance: RouDi_ID, Event: Port'
2026-01-14 20:30:25.659 [ Debug ]: Created new SubscriberPort for application 'introspection' with service description 'Service: Introspection, Instance: RouDi_ID, Event: PortThroughput'
2026-01-14 20:30:25.659 [ Debug ]: Created new SubscriberPort for application 'introspection' with service description 'Service: Introspection, Instance: RouDi_ID, Event: SubscriberPortsData'
```

So it's not giving `[ Error ]: Version mismatch from 'introspection'!` anymore. Good!

Also, `iox-introspection-client` now shows:
![[Pasted image 20260114203403.png]]

Run two ros2 nodes that uses shared memory:
![[Pasted image 20260114203655.png]]

And now `iox-introspection-client` looks like:
![[Pasted image 20260114203726.png]]

Now, need to learn how to use this introspection tool.



# CycloneDDS with Shared Memory Guide 

> [!NOTE] You can find this document here:
> `https://github.com/ros2/rmw_cyclonedds/blob/rolling/shared_memory_support.md`
> 
> But, I copy&pasted it here for readability
...
```
export CYCLONEDDS_URI=file:///home/ted/projects/standard_cyclonedds_test/cytclonedds.xml
